...
- **Paso 2 (Bienvenida oficial):** Cuando la opción está clara, entrega el mensaje oficial cargado en la app con saltos de línea prolijos, opciones de variantes con precios y la pregunta de variante. Vale igual para el SEGUNDO producto de una charla: un kit que el cliente todavía no vio se presenta con su ficha, no se cuenta en prosa. Lo que se adapta es el marco, no el cuerpo — la plantilla está escrita para el primer mensaje, así que `bloquePresentacionOficial` (`catalogo-precios.ts`) le saca el saludo, le saca la repregunta de la moto si el cliente ya la dijo, y manda la ficha en su propio globo. Sin eso el kit simple solo recibía "Mensaje oficial cargado en la app" (un dato más, no una orden de mandarlo) y el modelo lo resumía (conv 4387, 16/09).
...
- **Barridos deterministas de compatibilidad (sin LLM, solo BD):** `bot-agente/pruebas/sweep-compatibilidad.ts` compara los VEREDICTOS de todas las motos x todos los kits contra un snapshot (`--actualizar` para aceptar un cambio querido); `bot-agente/pruebas/barrido-detalle.ts` muestra el MOTIVO que acompaña a cada veredicto (sin snapshot, se lee a ojo con `diff`). Correr los dos después de tocar el matching de motos/kits o las filas de compatibilidad: el primero no ve un motivo ajeno pegado a un veredicto correcto, y el segundo es el que descubrió el precio de otro combo viajando en la respuesta del Kit 120.
