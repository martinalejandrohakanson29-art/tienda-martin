// app/admin/listas/articulos-importados/page.tsx
import { getArticulos } from "@/app/actions/costos";
import { getConfig } from "@/app/actions/config"; // Importante traer el config
import { ArticulosTable } from "./articulos-table";
import { Button } from "@/components/ui/button";
import { ArrowLeft } from "lucide-react";
import Link from "next/link";

export default async function ArticulosImportadosPage() {
  const data = await getArticulos();
  const config = await getConfig(); // Traemos los valores fijos de la DB

  return (
    <div className="flex flex-col h-screen">
      <div className="bg-white border-b px-8 py-4 flex items-center justify-between shadow-sm">
        <div className="flex items-center gap-4">
          <Link href="/admin/listas">
            <Button variant="outline" size="sm" className="gap-2 border-slate-200">
              <ArrowLeft className="h-4 w-4" />
              Volver a Listas
            </Button>
          </Link>
          <h2 className="text-2xl font-bold tracking-tight">Artículos Importados</h2>
        </div>
        <div className="text-sm text-slate-500 font-medium bg-slate-100 px-3 py-1 rounded-full">
          {data.length} Artículos cargados
        </div>
      </div>

      <div className="flex-1 overflow-auto p-8 pt-4">
        {/* Pasamos initialConfig a la tabla */}
        <ArticulosTable data={data} initialConfig={config} />
      </div>
    </div>
  );
}
