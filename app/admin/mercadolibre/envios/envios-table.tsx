...
export function EnviosTable({ envios }: EnviosTableProps) {
    ...
    const getLogisticConfig = (type: string) => {
        switch (type) {
            case 'self_service':
                return { label: "Envío Flex", className: "bg-orange-50 text-orange-700 border-orange-200" };
            case 'cross_docking':
                return { label: "Envío Colecta", className: "bg-blue-50 text-blue-700 border-blue-200" };
            default:
                    ...
                    {envios.map((envio) => {
                        ...
                        return (
                            <TableRow key={envio.id} className="hover:bg-slate-50/50 transition-colors border-b last:border-0">
                                ...
                                {/* CONTENIDO DE LA COLUMNA AGREGADOS */}
                                <TableCell className="px-4 py-4">
                                            ...
                                            <div key={item.id} className="text-[11px]">
                                                {item.agregadoInfo ? (
                                                    <div className="bg-slate-50 border border-slate-100 rounded p-1.5">
                                                        <div className="font-semibold text-slate-700 leading-tight mb-1">
