...
export async function GET(
  ...
  try {
    ...
    // 2. Obtener el pedido y su URL de PDF
    const venta = await prisma.venta.findUnique({
      where: { id },
      select: { pdfUrl: true, id: true, cliente: true }
    });

    if (!venta || !venta.pdfUrl) {
      return new NextResponse("Pedido o PDF no encontrado", { status: 404 });
    }
    ...
    // 4. Descargar de S3 usando el cliente que ya tiene rejectUnauthorized: false
    ...
    // 5. Preparar la respuesta para el navegador
    const contentType = response.ContentType || "application/pdf";
    const finalFileName = fileName || `pedido_${venta.cliente?.replace(/[^a-zA-Z0-9]/g, '_') || venta.id.slice(0, 8)}.pdf`;

    // Convertir el Body de S3 a un ReadableStream web de forma segura
