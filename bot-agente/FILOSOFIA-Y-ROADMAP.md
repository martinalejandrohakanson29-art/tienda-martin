...
- [x] **Fase 6: Pasaje global (n8n → bot-agente para TODAS las conversaciones)** — **ACTIVADO 07/09 ~14:30 AR**
  ...
  - **Pendientes de corregir EN VIVO** (el motor tiene menos kilómetros que n8n; se decidió ajustar con casos reales):
    - **Compatibilidad de más**: el scorer de `consultar_compatibilidad` confirma cuando comparten marca pero no modelo (ej: "Corven Energy 110" → fila de "Corven Triax 150", "Motomel s2" → "S2 190 cc") si además pega el nombre del kit. Y CB125F daba compatible con el Kit 170 (no es varillero). El fix real: que no confirme sin match fuerte de moto y que esos casos escalen.
