...
| "Este kit ya se presentó, no mandes la ficha de nuevo" | `chat_conversacion_estado` → `ContextoEjecucion.embudo` que reciben las tools de catálogo y variante | nada: el motor lo inyecta; el panel lo siembra cuando el equipo manda un kit a mano |
...
- **Preguntá solo en un idioma que después vas a entender.** Si `pregunta_variante_reintento` le ofrece al cliente identificar su variante por una pista (color, dientes, milímetros), esa pista TIENE que estar en `sinonimos_variante`. En la conv 3448 el bot ofreció el color, el cliente contestó "el cilindro es color plateado" y el bot le repitió la misma pregunta tres veces. Hay un check gratis que lo detecta: `node scratch/probar-mejoras.cjs` compara los números de la pregunta contra los sinónimos cargados.
- **El matcheo de motos por substring suelto confunde motos que no tienen nada que ver.** "nt" está dentro de "hu**nt**er" (una fila de Zanella NT 110 confirmaba un kit para una Corven Hunter 150) y el alias "s 2" de la Motomel S2 está dentro de "wave **s 2**022" (una fila de Honda Wave le negaba el kit a una Motomel S2). Regla: igualdad siempre; contención **solo entre tokens de 4+ caracteres** (`tokensDeModeloCoinciden`) y **solo respetando bordes de palabra** (`contieneComoTokens`). Auditoría permanente: `node scratch/auditar-compatibilidad.cjs` barre cada moto contra cada kit y falla si alguna respuesta cita una moto que no comparte ninguna palabra con la preguntada.
- **Una fila de compatibilidad que solo nombra la MARCA decide por toda la marca.** El nombre canónico de cada modelo la contiene ("Motomel Blitz 110".includes("motomel")), así que una fila "motomel → compatible" confirmaba el combo para cualquier Motomel, incluida una Skua 250. Ahora esas filas se ignoran salvo que el cliente tampoco haya dado un modelo. Regla general: **una confirmación de compatibilidad tiene que apoyarse en una fila tan específica como la moto que dijo el cliente.**
- **Lo que Chatwoot escribe no es lo que escribió el cliente.** "This message is unavailable." entró al motor como consulta y terminó en un veredicto de incompatibilidad. Los carteles de plataforma se filtran en `esPlaceholderDeChatwoot()`, en el webhook y en la transcripción.
...
- **Una tool que entrega el libreto de presentación lo va a entregar SIEMPRE, aunque el cliente ya lo haya recibido.** Conv 2763 (08/09): con el combo elegido, la moto confirmada y la variante resuelta, el cliente preguntó una sola cosa ("ya viene listo para colocar?") y le volvió la ficha entera (nombre, precio, envío gratis, lista de "qué incluye") más la foto. `consultar_catalogo_y_precios` devolvía la guía del PASO 2 ("mandá el mensaje oficial tal cual") porque no sabía en qué punto de la charla estaba. Es el mismo bug que el volcado repetido de envíos, con otra tool: el arreglo es el mismo patrón — el motor le pasa el punto del embudo por `ContextoEjecucion.embudo` (nunca por un argumento que el modelo pueda falsear) y la tool entrega **datos secos** en lugar del libreto. Idem `resolver_variante`, que ahora distingue "VARIANTE RESUELTA" de "VARIANTE YA RESUELTA DE ANTES".
- **La memoria de estado tiene que sembrarse también cuando interviene un humano.** En esa misma conv el bot arrancó con la memoria en blanco porque los dos mensajes previos del cliente los salteó (`/bot off`) y la respuesta la mandó el equipo desde el panel. Con el estado vacío, el turno siguiente era "primer contacto" para el motor: re-presentó todo y reenvió la foto (el candado de la foto se apoya en ese mismo estado). Ahora `enviarMensajeChatsVivo` con un kit seleccionado marca el combo/kit como presentado en `chat_conversacion_estado`. **No** marca la variante como resuelta a propósito: que el equipo mande la info de una variante no prueba que el cliente la haya elegido, y dar por definida la variante equivocada vende el producto equivocado.


- **Un mensaje del cliente que quedó sin respuesta no puede desaparecer del contexto.** Conv 3637 (08/09): el bot escaló bien "escape paolucci para varillero s2 motomel" (moto no registrada) y se quedó callado; el cliente insistió con "??" y le contestó *"Que info te falta del 170? Si me decis tu moto te confirmo compatibilidad al toque."* — la ficha de otro kit, de media hora antes, y sin llamar una sola herramienta. La causa no fue la ráfaga ni el debounce: el historial se armaba cortando en **el último mensaje saliente** (`transcripcion.slice(0, cortIdx + 1)`), así que la pregunta escalada — que por definición no tiene saliente después — no entraba ni al historial ni al mensaje del turno. El modelo vio literalmente `[kit 170 → ficha] + "??"` y contestó lo único que veía. Ahora el historial se arma con `recortarMensajesDelTurno()`: saca del final **solo** los mensajes que este turno va a responder, y todo lo demás queda. Regla: el corte del historial se define por *qué se está por responder*, nunca por *qué se respondió*.
- **Un escalado sin memoria dura un solo turno.** Escalar en silencio dejaba la fila en la bandeja del equipo pero **ningún rastro en la charla**: el turno siguiente arrancaba en blanco y el bot volvía a hablar por encima de algo que él mismo había derivado. Ahora el escalado se anota en `chat_conversacion_estado.escalado_pendiente_*` (vigencia 24hs) y sale en `MEMORIA DE ESTADO` como "consulta ya derivada, sin respuesta humana todavía"; se cierra sola cuando un compañero contesta en el chat. Encima, una insistencia pelada ("??", "hola?", "y?") con un escalado abierto se corta antes del modelo (`esInsistenciaSinContenido`): silencio a costo $0, porque no hay nada nuevo que contestar y lo que falta lo tiene que contestar un humano.
- **Dentro de una tool, el orden de los chequeos ES la regla de negocio.** Conv 3677 (08/09): combo elegido y ZB 110 confirmada, el bot preguntó el recorrido y el cliente contestó *"No como se eso ?"* → le devolvió **"Recorrido corto ($175.000) o Recorrido largo ($189.000)?"**, y lo repitió cuando insistió con *"nose si lo tengo corta o larga"*. Hubo que apagar el bot y contestar a mano. Nada de eso lo redactó el modelo: era, casi textual, el `mensaje_para_agente` de la rama "ambiguo" de `resolver_variante`. La causa es sólo de orden: el chequeo de *"el cliente nombró las dos variantes"* (`hits > 1`) corría **antes** que el de `cliente_no_sabe` y lo pisaba, así que `pregunta_variante_reintento` (la guía de cómo fijarse) nunca llegaba a servirse. Y el trigger es sistemático, no un caso raro: **un cliente que no sabe casi siempre nombra las dos opciones al negarlas** ("no sé si lo tengo corta o larga"), que para un matcher de presencia de palabra es idéntico a nombrarlas a propósito. Reglas: (a) cuando dos condiciones pueden ser ciertas a la vez, el orden entre ellas es una decisión de negocio y se documenta como tal — acá **el "no sé" le gana al "ambiguo"**; (b) una repregunta **no repite los precios que el cliente ya escuchó**: repetir el número en vez de ayudarlo a definir la variante es exactamente lo que se lee como "me volvió a tirar el precio". La rama ambigua ahora nombra las variantes sin precio y adjunta la guía si el grupo la tiene.

**Pendiente al cierre del 08/09:** el banco quedó en **40/41**. El caso que falla es `caso-34-rafaga-moto-mas-duda-tecnica` (Kit 170 + duda del cigüeñal): escaló como `consulta_tecnica` en vez de contestar. **No** pasa por el código de la conv 3677 (el Kit 170 es un pack suelto, `grupo_id NULL`, nunca entra a la rama `hits > 1`), así que es varianza del modelo o una regresión anterior — quedó sin diagnosticar. El caso nuevo `caso-42-no-sabe-la-variante-gana-sobre-ambiguo` se agregó al banco pero **todavía no se corrió**; el fix sí se verificó a mano contra la DB y el motor reales (los dos mensajes de la conv 3677 ahora devuelven la guía).
