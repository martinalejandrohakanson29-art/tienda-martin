...
export interface ConfiguracionAgente {
    ...
    /**
     * La confirmacion que manda el EQUIPO al cargar una compatibilidad positiva
     * desde el panel de escalados. No la usa el bot para redactar: lo que el
     * bot dice al confirmar por su cuenta sale de `chat_frases`.
     */
...
export async function obtenerConfiguracionAgente(): Promise<ConfiguracionAgente> {
    try {
        ...
        const mensajeCompatible = mapa.get("mensaje_compatible") || CONFIG_DEFAULTS.mensajeCompatible
