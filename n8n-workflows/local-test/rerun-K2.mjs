...
async function runScenario({ id, titulo, convId, phone, msgId, content, overrides = {}, waitCount = 1, timeoutMs = 150000, expectNoReply = false, dbAfter = null }) {
  ...
  let messages = expectNoReply ? await waitForNoMessage(convId, sinceId, Math.min(timeoutMs, 90000)) : await waitForMessages(convId, sinceId, waitCount, timeoutMs);
...
async function main() {
  db = new Client(DB);
  await db.connect();
  // limpiar residuo de la corrida anterior en estas 2 conversaciones (970, 971)
  await q("DELETE FROM conversaciones_historial WHERE session_key IN ('+5493520000970','+5493520000971')");
  await q("DELETE FROM preguntas_tecnicas_pendientes WHERE conversation_id IN ('970','971')");
  await q("DELETE FROM compatibilidades WHERE fuente LIKE '%970%' OR detalle LIKE '%zanella%'");
  await mockPostReset(970); await mockPostReset(971);

  await runScenario({ id: 'K64-paso1b', titulo: 'Cliente pregunta tecnica sin dato (crea pendiente)', convId: 970, phone: '+5493520000970', msgId: 22050,
    content: 'la zanella z110 recorrido largo anda con el kit 120?', waitCount: 1, timeoutMs: 220000,
    dbAfter: async () => ({ tecnicas: await q("SELECT * FROM preguntas_tecnicas_pendientes WHERE conversation_id='970'") }) });

  await runScenario({ id: 'K64-paso2b', titulo: 'Equipo responde la pendiente en PUBLICO (message_type outgoing correcto; respuesta publica NO deberia generar mensaje al cliente, ver punto 71)', convId: 970, phone: '+5493520000970', msgId: 22051,
    content: 'si, la zanella z110 recorrido largo anda perfecto con el kit 120', overrides: { senderType: 'team' }, expectNoReply: true, timeoutMs: 220000,
    dbAfter: async () => ({ tecnicas: await q("SELECT * FROM preguntas_tecnicas_pendientes WHERE conversation_id='970'"), compat: await q('SELECT * FROM compatibilidades') }) });

  await runScenario({ id: 'K64-paso3b', titulo: 'Cliente escribe despues -> el bot NO debe estar pausado', convId: 970, phone: '+5493520000970', msgId: 22052,
    content: 'joya, gracias!', waitCount: 1, timeoutMs: 220000 });

  await runScenario({ id: 'K68b', titulo: 'Equipo responde algo que no corresponde a ninguna pendiente (outgoing correcto)', convId: 971, phone: '+5493520000971', msgId: 22060,
    content: 'dale, cualquier cosa me avisas', overrides: { senderType: 'team' }, expectNoReply: true, timeoutMs: 120000 });

  await runScenario({ id: 'K68b-verif', titulo: 'Cliente escribe justo despues -> deberia quedar en silencio (pausado)', convId: 971, phone: '+5493520000971', msgId: 22061,
    content: 'hola, cuanto sale el kit 120?', expectNoReply: true, timeoutMs: 120000 });
